...
async function requireAdmin(req: Request) {
  ...
  if (!session) return { error: Response.json({ error: "Please log in." }, { status: 401 }) };
...
export async function POST(req: Request) {
  ...
  if (!entity || !(entity in schemas)) {
    return Response.json({ error: `Unknown entity: ${String(body.entity)}` }, { status: 400 });
  }

  const parsed = schemas[entity].safeParse(body.data);
  if (!parsed.success) {
    const issue = parsed.error.issues[0];
    return Response.json({ error: `${issue.path.join(".")}: ${issue.message}` }, { status: 422 });
  }
  ...
  try {
    switch (entity) {
      case "campus": {
        ...
        return Response.json({ success: true, id: String(res.insertedId), message: `Campus "${d.name}" created.` });
      }
      case "trainer": {
        ...
        return Response.json({ success: true, id: String(res.insertedId), message: `Trainer "${d.name}" added.` });
      }
      case "course": {
        ...
        return Response.json({ success: true, id: String(res.insertedId), message: `Course "${d.name}" created.` });
      }
      case "student": {
        ...
        return Response.json({ success: true, id: String(ind.insertedId), message: `Student "${d.name}" enrolled.` });
      }
      case "active_class": {
        ...
        return Response.json({ success: true, id: String(res.insertedId), message: `Class "${d.name}" scheduled.` });
      }
    }
  } catch (error) {
