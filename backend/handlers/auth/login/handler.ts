import { sessionCookie } from "@/backend/auth/session";
import bcrypt from "bcryptjs";
import { z } from "zod";
import { mongo } from "@/lib/mongodb";
import { signSession, SESSION_MAX_AGE } from "@/lib/auth-jwt";
...
export async function POST(req: Request) {
  ...
  const res = Response.json({ session: account.session, token });
