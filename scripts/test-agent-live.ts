...
async function main() {
  ...
  try {
        ...
        messages: [
          {
            role: "user",
            content:
              "Campus-wise enrolments ka bar chart bana do aur simple words mein batao sab se zyada enrolments kahan hain.",
          },
        ],
      }),
    ...
    console.log(
      "PASS live AI query → verified chart → explanation → persisted chat history",
      result.model,
      summary.ms + "ms",
    );
  } finally {
